...
Um Zeiger verstehen zu können, muss man sich zunächst klar machen, das eine Variable im Prinzip aus zwei Dingen besteht:
...
Den subtile Unterschied kann man am ehesten damit erklären, dass \verb|n| konstant ist, also die Adresse nicht verändert werden darf.
Das bedeutet, dass folgendes \texttt{C} Quelltext korrekt ist
\begin{lstlisting}
  int n[] = {1, 2, 3, 4, 5};
  int * p = n;
\end{lstlisting}
...
\end{lstlisting}
An der Kommandozeile kann man mit Hilfe der Funktionsargument von \verb|main| dem Programm Parameter übergeben.

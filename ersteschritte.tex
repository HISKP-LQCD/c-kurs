...
Dafür führen wir zunächst Datentypen in C ein, damit wir die Liste $\{x_i\}$ auf dem Rechner darstellen können. 
...
Bevor wir C-Datentypen vorstellen, ist es hilfreich zu verstehen, wie Daten allgemeine auf einem Rechner gespeichert werden.
...
\begin{lstlisting}[caption={Ein erstes C-Programm}, belowcaptionskip=0.3em]
int main()
{
...
Jedes Programm in C muss die Funktion \texttt{main} genau einmal definieren. 
Die \texttt{main} Funktion ist auch der Startpunkt für jedes C Programm.
...
Soweite als Erklärung für das erste Programm.
Wie übersetzt man nun dieses Programm?
Mit Übersetzen bezeichnet man den Schritt, in dem der C Quelltext in sogenannten Maschinentext übersetzt wird.
Maschinentext kann dann direkt von einem Rechner interpretiert werden.
Wir zeigen das Übersetzen beispielhaft für Linux und den GNU C Compiler \texttt{gcc}.
...
Denn \texttt{printf} ist nicht Teil der C Sprache, es ist in einer sogenannten Bibliothek definiert. 

...
\todo[inline]{BaKo: ich bin mir hier etwas unschlüssig, ob es nicht an dieser Stelle schon nützlich wäre, die Art und Weise zu erklären, wie der Compiler diese Dinger abspeichert...}
...
Man spricht auch von \emph{null-terminiert} oder \emph{0-terminiert}.
...
Hierfür stehen in \texttt{C} die beiden Funktionen \texttt{sprintf} und \texttt{snprintf} zur Verfügung, welche wie die schon bekannte \texttt{printf} Funktion funktioniern.\index{\texttt{sprintf}}\index{\texttt{snprintf}}
Im Gegensatz zu \texttt{printf}, schreiben diese Funktionen jedoch direkt in eine sich im Speicher befindliche Zeichenkette.

\todo[inline]{BaKo: Ich werde hier wohl noch auf die Signatur von sprintf und snprintf eingehen. Das Beispiel möchte ich lesbarer machen. Meines Erachtens ist \texttt{snprintf} wirklich sehr wichtig und ich möchte, dass der Punkt verstanden wird. Da die Vorlesung jetzt nicht so dicht gepackt ist, wird dies wohl kein Problem darstellen.}
...
int main()
{
  ...
  // kurze Zeichenkette wird in short_string geschrieben, Rueckgaberwert wird ueberprueft
